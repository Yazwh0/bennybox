...
3. Each release is signed with its own build-time key, not a persistent one, so updating currently means uninstalling the previous APK first - Android won't install over a different signing key.
